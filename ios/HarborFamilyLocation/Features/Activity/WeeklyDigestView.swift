    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                Text("\(HarborFamilyLocationSample.digestRange) · \(HarborFamilyLocationSample.digestCircleName)")
                    .font(.system(size: 14))
                    .foregroundStyle(.secondary)

                HStack(spacing: 10) {
                    statTile(value: "\(HarborFamilyLocationSample.digestCheckIns)", label: "check-ins sent")
                    statTile(value: "\(HarborFamilyLocationSample.digestPlaces)", label: "places visited")
                    statTile(value: "\(HarborFamilyLocationSample.digestAlerts)", label: "alert · low battery")
                }
                .padding(.top, 14)
                ...
                VStack(spacing: 14) {
                    ForEach(HarborFamilyLocationSample.digestMembers) { member in
                        memberCard(member)
                    }
                }
            }
