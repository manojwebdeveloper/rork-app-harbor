...
final class AuthService: ObservableObject {
    ...
    var displayName: String {
        ...
        return user?.isAnonymous == true ? "Guest" : "HarborFamilyLocation member"
    }
    ...
    func signInAsGuest() async {
        ...
        do {
            ...
            NSLog("[HarborFamilyLocationAuth] Guest sign-in succeeded uid=%@", authResult.user.uid)
            ...
            do {
                ...
                NSLog("[HarborFamilyLocationAuth] Guest profile write skipped: %@", error.localizedDescription)
            }
        } catch {
            NSLog("[HarborFamilyLocationAuth] Guest sign-in failed: %@", (error as NSError).description)
            errorMessage = readableMessage(for: error)
        }
    }
