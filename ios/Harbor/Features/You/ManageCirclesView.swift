...
struct ManageCirclesView: View {
    @State private var sharingByCircleID: [String: Bool] = Dictionary(
        uniqueKeysWithValues: HarborSample.circles.map { ($0.id, true) }
    )
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("\(sharingCount) of \(HarborSample.circles.count) circles sharing your location")
                    .font(.system(size: 14))
                    .foregroundStyle(.secondary)

                ForEach(HarborSample.circles) { circle in
                    card(for: circle)
                }
