...
final class AuthSession: ObservableObject {
    ...
    var displayName: String {
        ...
        return user?.isAnonymous == true ? "Guest" : "Harbor member"
    }
    ...
    func signInAsGuest() async {
        ...
        do {
            ...
            NSLog("[HarborAuth] Guest sign-in succeeded uid=%@", authResult.user.uid)
            ...
            do {
                ...
                NSLog("[HarborAuth] Guest profile write skipped: %@", error.localizedDescription)
            }
        } catch {
            NSLog("[HarborAuth] Guest sign-in failed: %@", (error as NSError).description)
            errorMessage = readableMessage(for: error)
        }
    }
